...
### 3. 발송과 적용은 다르다 — PassRole 작성기 검증

승인이 PassRole을 부여하면, 사람마다 작업 지시 하나가 마커 버킷에 나가고 그것을 받아 권한 세트의
인라인 정책에 문장을 쓰는 것은 **다른 컨테이너**다. 적용기의 기록에는 `dispatched`라고만 적히고,
그것은 **작업 지시를 썼다**는 뜻이지 문장이 들어갔다는 뜻이 아니다.

그 차이가 조용한 실패를 하나 만들었다. 세 명분을 확인한 승인이 적용되고 그중 한 작성기가 실패하면,
화면은 「적용 완료」라고만 말했다. 요청한 사람은 권한을 받지 못했고, 이유는 CloudWatch에 있었으며,
남은 흔적은 아무도 읽지 않는 잠금 객체 하나뿐이었다.

이제 계획 화면의 PassRole 구역이 세 객체를 대조한다.

| 무엇인가 | 어디 있나 | 누가 쓰나 |
|---|---|---|
| 이 결정이 발송한 작업 지시 목록 | 상태 버킷 `<계정>/<자원>/plan/outcome.json`의 `passrole_dispatch` | 적용기 |
| 작성기가 그 지시로 무엇을 했는가 (`written`·`refused`·`failed`와 사유) | 마커 버킷 `inline_result/<계정>:<권한 세트>.json` | 인라인 작성기 |
| 그 권한 세트에 아직 끝나지 않은 실행이 있는가 (곧 잠금) | 마커 버킷 `inline_writer/<계정>:<권한 세트>.json` | 적용기가 쓰고 작성기가 지운다 |

**발송 n건 · 적용 n건**이 그 문장이고, 두 수가 다르면 그만큼은 승인만 되고 권한은 들어가지 않은
것이다. 실패한 대상에는 원인이 함께 나오고 「다시 보내기」가 붙는다.

다시 보내는 것은 **계획을 다시 적용하지 않는다.** 저장된 계획 파일은 두 번 돌지 않으므로 — terraform은
상태가 움직인 뒤의 저장 계획을 거부한다 — 적용기는 그때 기록해 둔 출력값으로 작업 지시만 새로 쓴다.
잠금을 넘겨받는 것은 작성기가 **그 실행은 끝났고 쓰지 못했다**고 남긴 경우뿐이다. 기록이 없는
잠금은 재시도 대상이 아니다 — 돌고 있는 작업과 죽은 작업이 같은 객체를 남기기 때문이다.
...
**호스트 키를 등록하지 않는다.** `known_hosts`는 주소를 키로 삼기 때문에 인스턴스를 교체하거나
주소가 바뀔 때마다 낡는다 — 배포와 아무 상관 없는 이유로 배포가 깨진다. 그래서 실행마다 비어 있는
`known_hosts`에 `accept-new`로 붙는다. 같은 실행 안의 두 번째 연결부터는 첫 번째가 기록한 키로
검증되므로, `scp`와 `ssh` 사이에 호스트가 바뀌는 것은 여전히 잡힌다.

무엇을 잃는지 정확히 적는다. **공개 키 인증은 개인 키를 전송하지 않는다** — 시도값에 서명할 뿐이다.
따라서 적대적 호스트가 배포 키를 얻지는 못한다. 얻는 것은 묶음 파일과 원격 스크립트인데, 전자는
저장소 내용이고 후자는 커밋 해시만 싣는다. `OPT_DASHBOARD_API_KEY`는 호스트의 환경 변수 파일에
있고 **이 워크플로는 그 파일을 만들지 않는다.**

잃는 것은 **배포가 의도한 기계에 도달했다는 보장**이다. 그것은 아래 신원 확인이 대신 맡는다.

환경에 **Required reviewers**를 걸면 이 파일을 고치지 않고도 배포에 승인 단계가 생긴다.

#### 배포가 올바른 기계에 도달했는지 어떻게 아는가

설치 전에 인스턴스 메타데이터에서 **인스턴스 프로파일 이름**을 읽어 `opt-SolutionDashboard`인지
확인한다. 아니면 그 자리에서 실패한다.

호스트 키가 하던 일과 다른 질문에 답한다는 점이 요점이다. 호스트 키는 "지난번과 같은 기계인가"를,
인스턴스 프로파일은 "대시보드 역할을 가진 기계인가"를 묻는다. **후자는 인스턴스 교체와 주소 변경을
견딘다** — 전자가 계속 낡던 바로 그 이유다.

이것은 **잘못된 주소**를 잡는 것이지 적대적 호스트를 잡는 것이 아니다. 상대가 호스트를 장악했다면
메타데이터도 원하는 대로 답할 수 있다. 대신 리스너 호스트에 대시보드를 배포하는 사고는 여기서
멈춘다 — 솔루션의 EC2 인스턴스는 둘뿐이고 그 주소가 나란한 비밀값에 들어 있다.
...
| 액션이 `wrong host: expected instance profile ...`로 실패 | 주소가 다른 인스턴스를 가리킨다 | `DASHBOARD_SSH_HOST` 값과 실제 인스턴스 대조 |
| 액션이 `no instance metadata answered`로 실패 | EC2가 아니거나 IMDSv2가 꺼져 있다 | 인스턴스 메타데이터 설정 확인 |
